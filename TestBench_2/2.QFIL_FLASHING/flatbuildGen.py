...
################## --- Inputs ---- ##########################################
# 'systemdata','vcpu', 'nonhlos',
listOfFlashItems = ['richos', 'qnx', 'vcpu', 'nonhlos']
...
#inputArtifactoryPath = "https://artifact.swf.daimler.com:443/artifactory/apricotbscqal/build/system.apricot-system-manifests.release_E030.promote"
kdsFileName = "7513700148.KDS"
...
def downloadArtifacts(bdType, listOfFlashItems, bdNumber, artifactoryPath, otPath, userName, artifactoryToken):
	if bdType == 'systemBundle':
		...
		if variant == '8195c':
			...
			inp = artifactoryPath + "/" + \
				str(bdNumber) + "/" + "archive_binz-apricot-image-ui-dev"
			out = os.path.abspath(
				otPath + "\\" + "archive_binz-apricot-image-ui-dev" + ".zip")
		path = ArtifactoryPath(inp, auth=(userName, artifactoryToken))
		path.archive().writeto(out=out, chunk_size=16 * 1024 * 1024)
...
def copyRichosPackage(richosOutputPath, outputPath):
	...
	if variant == '8195c':
		...
		searchfile = "archive_binz-apricot-image-ui-dev.zip"
			...
			if key != '' and value != '':
				...
				try:
					...
					p = subprocess.run([qFilPath, '-COM=' + comPortNumber, '-Mode=3', '-FLATBUILDPATH=' + flashPath,
										...
										'-Patch=patch0.xml,patch1.xml,patch2.xml,patch3.xml,patch4.xml,patch5.xml,patch6.xml,patch7.xml',
										'-RawProgram=rawprogram0.xml,rawprogram1.xml,rawprogram2.xml,rawprogram3.xml,rawprogram4.xml,rawprogram5.xml,rawprogram6.xml,rawprogram7.xml',
