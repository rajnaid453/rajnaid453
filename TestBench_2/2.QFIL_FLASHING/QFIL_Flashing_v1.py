...
################## --- Inputs ---- ##########################################
listOfFlashItems = ['richos', 'vcpu', 'nonhlos', 'qnx', 'systemdata']  # 'vcpu'
variant = str(sys.argv[1])  # Ex: 8195c, 8295c, 8295b
# Ex: systemBundle, richos, richosWithSystemBundle
buildType = str(sys.argv[2])
buildNumber = int(sys.argv[3])  # Ex: 1810, 1752
richosBuild = int(sys.argv[4])  # Ex: 55100,61630
userName = str(sys.argv[5])  # Ex: nachakr
...
# inputArtifactoryPath = "https://artifact.swf.daimler.com:443/artifactory/apricotbscqal/build/system.apricot-system-manifests.release_E025_0.promote"
kdsFileName = "7513700051.KDS"
...
if buildType == "systemBundle":
	...
	if os.path.exists(richosSystemBundleOutputPath):
		...
		if call == 1 and richosCall == 0 and systemBundleCall == 0:
			print("Parent dirctory already exists, children: system bundle, richos Directories are created")
			os.makedirs(richosOutputPath)
			os.makedirs(systemBundleOutputPath)
		if call == 1 and richosCall == 2 and systemBundleCall == 0:
			print("Parent and child Richos Directory Already Exist, system bundle directory is created")
			os.makedirs(systemBundleOutputPath)
		if call == 1 and richosCall == 0 and systemBundleCall == 2:
			print("Parent and child System bundle Directory Already Exist, richos directory is created")
			os.makedirs(richosOutputPath)
		if call == 1 and richosCall == 2 and systemBundleCall == 2:
			print("Parent and children richos,system bundle Directories Already Exist")
...
def downloadArtifacts(bdType, listOfFlashItems, bdNumber, artifactoryPath, otPath, userName, artifactoryToken):
	if bdType == 'systemBundle':
				...
				if variant == '8195c':
					...
					inp = artifactoryPath + "/" + \
					    str(bdNumber) + "/" + item + "/" + "dev" + \
					        "/" + "archive_balboa-apricot-image-ui"
					out = os.path.abspath(
					    otPath + "\\" + "archive_balboa-apricot-image-ui" + ".zip")
				elif variant == '8295c':
					print("Downloading {} Artifacts Started".format(item))
					inp = artifactoryPath + "/" + \
					    str(bdNumber) + "/" + item + "/" + "dev" + \
			...
			if item == 'richos':
				...
				if variant == '8195c':
					...
					inp = artifactoryPath[-1] + "/" + \
					    str(bdNumber[-1]) + "/" + "archive_balboa-apricot-image-ui"
					out = os.path.abspath(otPath[-1] + "\\" +
					                      "archive_balboa-apricot-image-ui" + ".zip")
				elif variant == '8295c':
					print("Downloading {} Artifacts Started".format(item))
					inp = artifactoryPath[-1] + "/" + \
					    str(bdNumber[-1]) + "/" + "archive_binz-apricot-image-ui"
					out = os.path.abspath(otPath[-1] + "\\" +
		...
		if variant == '8195c':
			inp = artifactoryPath + "/" + \
			    str(bdNumber) + "/" + "archive_balboa-apricot-image-ui"
			out = os.path.abspath(
			    otPath + "\\" + "archive_balboa-apricot-image-ui" + ".zip")
		else:
			inp = artifactoryPath + "/" + \
...
def unzipArtifacts(listOfFlashItems, outputPath):
	...
	for item in listOfFlashItems:
		if item == 'richos':
			if variant == '8195c':
				zipPath = outputPath + "\\" + "archive_balboa-apricot-image-ui" + '.zip'
				unzipPath = outputPath + "\\" + item + "\\" + "archive_balboa-apricot-image-ui"
			elif variant == '8295c':
...
def moveqnx():
	if buildType == "richosWithSystemBundle":
		...
		destination = os.path.abspath(os.path.join(systemBundleOutputPath, "qnx\\"))
	else:
				...
				for line in data:
					...
					if text in line:
						...
						if text.capitalize() in line:
							print("Parsing of Shell file text {} Started".format(text.capitalize()))
							line = line.replace(text.capitalize(), newText)
							print("Parsing of Shell file text {} Completed".format(text.capitalize()))
						f1.write(line)
						i = i + 1
...
def copyRichosPackage(richosOutputPath, outputPath):
	...
	if variant == '8195c':
		searchfile = "archive_balboa-apricot-image-ui.zip"
	else:
...
def flashFlatBuild(outputPath, qFilPath):
	if os.path.exists(os.path.join(outputPath, "nonhlos", "flat")):
		flashPath = os.path.abspath(os.path.join(outputPath, "nonhlos", "flat"))
		...
		while (saharaCount < retry):
			...
			if key != '' and value != '':
				...
				try:
                    ...
                    p = subprocess.run([qFilPath, '-COM=' + comPortNumber, '-Mode=0', '-FLATBUILDPATH=' + flashPath,
...
def writeBuildInformation(outputPath):
	...
	with open(filePath, 'w') as f:
		...
		if sys.argv[1] == "richos":
			...
			f.write("\nRichOS Artifactory Path: {}".format(richosArtifactoryPath))
			f.write("\nRichOS Output Path: {}".format(richosOutputPath))
		elif sys.argv[1] == "richosWithSystemBundle":
			f.write("\nRichOS buildNumber: {}".format(richosBuild))
			f.write("\nRichOS Artifactory Path: {}".format(richosArtifactoryPath))
			f.write("\nSystem bundle Information: {}".format(inputArtifactoryPath))
